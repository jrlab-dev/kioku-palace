...
import {ITEMS,getCourse,getPoint,getItem} from './route.js?v=trail6-20260925-final2';
...
export async function createPalaceScene({container,reduced=false,onProgress=()=>{},onError=()=>{},onStats=()=>{}}){
  let renderer,scene,camera,model,sun,travel=null,last=0,lookYaw=0,lookPitch=0,drag=null;
  let currentReduced=reduced,elapsed=0,frames=[],frameElapsed=0,ring,ring2;
  const items=[],itemsById=new Map(),templates=[],thumbnails=[];
  ...
  try{
    ...
    sun=new THREE.DirectionalLight(0xffe4bc,2.6);sun.position.set(-10,18,5);sun.target.position.set(0,0,-7);sun.castShadow=true;
    ...
    const gltf=await loader.loadAsync('./assets/shrine_learning.glb?v=trail6-20260925-final2',progress=>{if(progress.total){stats.assetBytes=progress.total;onProgress(Math.round(progress.loaded/progress.total*100));}});
    ...
    ITEMS.forEach(item=>{
      ...
      const point=getPoint(item.placeId),baseScale=item.baseScale;copy.scale.setScalar(baseScale);copy.position.copy(vec(point.scene.itemPosition));
  ...
  async function moveTo(point){
    ...
    if(innerWidth<640){
      ...
      if(point.id==='trailhead'){
        target.set(itemTarget.x,point.elevation+.30,itemTarget.z);
        position.y=Math.max(position.y,point.elevation+2.75);
      }
      position.add(position.clone().sub(target).normalize().multiplyScalar(.85));
      if(point.id==='trailhead')position.z=Math.min(position.z,-11.80);
    }
    sun.target.position.copy(target);sun.position.copy(target).add(new THREE.Vector3(-10,18,12));sun.target.updateMatrixWorld();
  ...
  function pair(setId,index){const course=getCourse(setId),item=getItem(course.itemIds[index]),point=getPoint(course.pointIds[index]);return {item,point,object:itemsById.get(item.id)};}
  function setItem(setId,index,visible,animate=false){const selected=pair(setId,index);items.forEach(item=>item.visible=visible&&item===selected.object);if(visible){const item=selected.object;item.position.copy(vec(selected.point.scene.itemPosition));item.scale.setScalar(item.userData.baseScale);item.rotation.set(0,0,0);item.userData.association=null;item.userData.base=item.position.y;item.userData.motion=animate&&!currentReduced?1:0;}}
  function playAssociation(setId,index,variant,animate=true){setItem(setId,index,true,false);const selected=pair(setId,index),item=selected.object,base=item.userData.baseScale;let from=item.position.clone(),to=item.position.clone(),scaleFrom=base,scaleTo=base,roll=false,sway=false,hop=false;
    if(selected.item.id==='milk'){from=new THREE.Vector3(-.35,.36,2.85);to=vec(selected.point.scene.itemPosition).add(new THREE.Vector3(0,.18,0));scaleFrom=.38;scaleTo=2.35;}
    else if(selected.item.id==='apple'&&variant==='roll'){from=new THREE.Vector3(1.20,1.93,-3.66);to=new THREE.Vector3(2.92,1.93,-3.66);scaleFrom=scaleTo=1.8;roll=true;}
    else if(selected.item.id==='apple'){from=new THREE.Vector3(2.35,1.50,-3.66);to=new THREE.Vector3(2.35,2.67,-3.76);scaleFrom=.65;scaleTo=2.65;}
    else if(selected.item.id==='umbrella'){from=to.clone().add(new THREE.Vector3(.65,-.25,.35));scaleFrom=.35;scaleTo=2.05;sway=true;}
    else if(selected.item.id==='boots'){from=to.clone().add(new THREE.Vector3(-1.1,-.2,.65));to=to.clone().add(new THREE.Vector3(.15,.15,-.1));scaleFrom=scaleTo=1.75;hop=true;}
    else if(selected.item.id==='bell'){from=to.clone().add(new THREE.Vector3(0,-.65,.05));scaleFrom=.7;scaleTo=2.25;sway=true;}
    else{sway=true;from=to.clone().add(new THREE.Vector3(0,-.42,.05));scaleFrom=.8;scaleTo=2.15;}
    ...
    item.position.copy(from);item.scale.setScalar(scaleFrom);item.userData.association={from,to,scaleFrom,scaleTo,roll,sway,hop,time:0,duration:selected.item.id==='milk'?1.55:1.25};
  }
  function hideItems(){items.forEach(item=>item.visible=false);}
  function mark(point,visible=true){ring.visible=visible;if(!visible)return;ring.position.copy(vec(point.scene.position));ring.position.y+=.06;}
  function showOverview(){if(travel)travel.resolve();view.position.copy(overview.position);view.target.copy(overview.target);sun.target.position.copy(overview.target);sun.position.copy(overview.target).add(new THREE.Vector3(-10,18,12));sun.target.updateMatrixWorld();lookYaw=lookPitch=0;hideItems();mark(0,false);}
